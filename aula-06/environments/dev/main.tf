# ============================================================
# Ambiente DEV — composicao dos modulos TechNova
# ============================================================
...
module "rds_sg" {
  ...
  vpc_id       = module.vpc.vpc_id   # composicao
  project_name = var.project_name
  environment  = var.environment
  ...
  ingress_rules = [
    {
      ...
      protocol        = "tcp"
      cidr_blocks     = []
      security_groups = [module.api_sg.sg_id]   # composicao: output do api_sg
    }
  ]
}
...
module "api_server" {
  ...
  subnet_id          = module.vpc.public_subnet_ids[0]   # composicao
  security_group_ids = [module.api_sg.sg_id]             # composicao
  key_name           = "vockey"
  project_name       = var.project_name
  environment        = var.environment
...
module "database" {
  ...
  subnet_ids         = module.vpc.private_subnet_ids    # composicao
  security_group_ids = [module.rds_sg.sg_id]            # composicao
